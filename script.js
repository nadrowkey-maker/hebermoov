...
const DATABASE = {
    parcours: [
        {
            id: 1, nom: "Forêt des Gayeulles", type: "Dense", niveau: "Intense", temps: "45m", kcal: 450,
    ...
    articles: {
        fondamentaux: `
            ...
            <p class="text-muted mb-15 line-height-15">L'Hébertisme repose sur un principe fondateur : le développement complet et utilitaire du corps humain. La musculation isolée n'a pas sa place ici.</p>
            ...
            <p class="text-muted line-height-15">L'objectif final se résume par la célèbre devise de Georges Hébert : <br><br><em class="text-neon border-l-neon pl-15 block">"Être fort pour être utile."</em></p>
        `,
        physio: `
            <h2 class="text-neon mb-15 mt-20 font-heading text-xl">Physiologie de l'Effort Naturel</h2>
            <p class="text-muted mb-15 line-height-15">S'entraîner sur un sol instable (forêt, boue, rochers) est fondamentalement différent d'une course sur tapis ou d'un exercice sur machine guidée.</p>
        ...
        survie: `
            ...
            <p class="text-muted mb-15 line-height-15">La forêt n'est pas aseptisée. Elle impose le respect de certaines règles de sécurité vitales.</p>
            <ul class="text-muted mb-20 line-height-15 pl-20">
        ...
        histoire: `
            ...
            <p class="text-muted mb-15 line-height-15">Georges Hébert (1875-1957) était un officier de la marine française. Son déclic a lieu en 1902, lors de la terrible éruption de la Montagne Pelée en Martinique.</p>
        ...
        nutrition: `
            ...
            <p class="text-muted mb-15 line-height-15">L'effort explosif en extérieur nécessite une nutrition stratégique.</p>
    ...
    feedSocial: [
        { user: "Léa M.", avatar: "L", action: "Super session équilibre ce matin sur les troncs abattus ! Le terrain était super glissant mais ça fait bien travailler la proprioception. 🌲🔥", park: "Forêt de Brocéliande", time: "Il y a 2 heures", likes: 12, likedByMe: true, img: "https://images.unsplash.com/photo-1448375240586-882707db888b?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=60" },
        { user: "Maxime D.", avatar: "M", action: "Qui est chaud pour un entraînement franchissement ce week-end ? J'ai repéré un super spot avec des dénivelés impressionnants. 💪", park: "Parc des Gayeulles", time: "Hier", likes: 5, likedByMe: false, img: "" },
        { user: "Marc (Coach)", avatar: "M", action: "Séance de portage lourd extrême aujourd'hui.", park: "St-Martin", time: "Il y a 2 jours", likes: 56, likedByMe: false, img: "https://images.unsplash.com/photo-1511497584788-876760111969?w=500&q=80" }
    ]
};
...
class AppState {
    constructor() {
        ...
        // Données d'exécution (non sauvegardées dans le cache)
        this.runtime = {
    ...
    getDefaultState() {
        return {
            ...
            // Les nouvelles données de ton Profil
            profile: {
                pseudo: 'Flavien R.',
                bio: 'L2 Maths • Rennes',
                currentWeight: ''
            ...
            settings: {
                lightMode: false, // Thème Nuit (false) ou Clair (true)
    ...
    loadState() {
        try {
            ...
            if (saved) {
                ...
                // Rétrocompatibilité : si c'est un vieux profil, on injecte les nouveautés
                if (!parsed.profile) parsed.profile = { pseudo: 'Flavien R.', bio: 'L2 Maths • Rennes', currentWeight: '' };
                if (parsed.settings.lightMode === undefined) parsed.settings.lightMode = false;
                return parsed;
            }
        } catch (e) {
...
function saveWeight() {
    const cw = document.getElementById('profile-current-weight').value;
    if (cw) {
        STORE.data.profile.currentWeight = cw;
        STORE.saveState();
        triggerToast("Poids enregistré", "Ton évolution a bien été sauvegardée.");
    }
}

// --- Mise à jour du Profil (Pseudo / Bio) ---
function saveProfileData() {
    const pseudo = document.getElementById('input-edit-pseudo').value;
    const bio = document.getElementById('input-edit-bio').value;
    
    if (pseudo) STORE.data.profile.pseudo = pseudo;
    if (bio) STORE.data.profile.bio = bio;
    
    STORE.saveState();
    closeOverlay('edit-profile-modal');
    triggerToast("Profil mis à jour", "Tes informations ont été modifiées.");
}

// --- Thème Mode Nuit / Clair ---
function toggleTheme() {
    STORE.data.settings.lightMode = !STORE.data.settings.lightMode;
    applyTheme();
    STORE.saveState();
}
...
function applyTheme() {
    ...
    if (STORE.data.settings.lightMode) {
        document.body.classList.add('light-mode');
        if (themeToggle) themeToggle.classList.remove('active'); // Bouton switch gris
    } else {
        document.body.classList.remove('light-mode');
        if (themeToggle) themeToggle.classList.add('active'); // Bouton switch vert
    }
}
...
function updateAllUI() {
    ...
    // Profil Général (Textes)
    const elPseudo = document.getElementById('display-pseudo');
    const elBio = document.getElementById('display-bio');
    const elWeight = document.getElementById('profile-current-weight');
    
    if (elPseudo && data.profile.pseudo) elPseudo.innerText = data.profile.pseudo;
    if (elBio && data.profile.bio) elBio.innerText = data.profile.bio;
    if (elWeight && data.profile.currentWeight) elWeight.value = data.profile.currentWeight;

    // Profil Général (Stats & Niveaux)
...
function buyItem(itemName, price) {
    if (STORE.data.inventory.includes(itemName)) {
        triggerToast("Erreur", "Vous possédez déjà cet équipement.");
        return;
    }
    if (STORE.data.xp >= price) {
        ...
        STORE.saveState();
        triggerToast("Achat Réussi", `L'objet "${itemName}" a été ajouté.`);
    } else {
        const missing = price - STORE.data.xp;
        triggerToast("Fonds Insuffisants", `Il manque ${missing} XP.`);
    }
}
...
function openArticle(articleKey) {
    ...
    document.getElementById('subpage-header-title').innerText = "Académie";
...
function renderSocialFeed() {
    ...
    DATABASE.feedSocial.forEach(post => {
        ...
        const imgHtml = post.img ? `<div class="post-img w-100 h-150 rounded-xl bg-cover bg-center mb-15 border-glass" style="background-image: url('${post.img}');"></div>` : '';
        ...
        const html = `
            <div class="feed-post glass-mirror p-20 rounded-2xl">
                ...
                <div class="post-actions flex-align gap-20 border-t-glass pt-15">
                    <button class="icon-btn flex-align gap-5 cursor-pointer hover-bg-light p-5-10 rounded-lg transition-bg" style="background:transparent; border:none;">
                        <i class="${heartClass}"></i> <span class="text-muted text-sm font-bold">${post.likes}</span>
                    </button>
                    <button class="icon-btn flex-align gap-5 cursor-pointer hover-bg-light p-5-10 rounded-lg transition-bg" style="background:transparent; border:none;" onclick="triggerToast('Bientôt', 'Commentaires disponibles à la MAJ.')">
    ...
    switch(pageType) {
        case 'history':
            ...
            if (STORE.data.history.length === 0) {
                htmlContent = `<div class="flex-col flex-center text-center mt-50 text-muted"><i class="fas fa-ghost text-5xl mb-20 opacity-50"></i><p>Vous n'avez pas encore affronté la forêt.</p></div>`;
            } else {
        ...
        case 'inventory':
            ...
            if (STORE.data.inventory.length === 0) {
                htmlContent = `<div class="flex-col flex-center text-center mt-50 text-muted"><i class="fas fa-box-open text-5xl mb-20 opacity-50"></i><p>Votre sac est vide. Allez dans Défis pour vous équiper.</p></div>`;
            } else {
...
function resetAppConfirm() {
    if (confirm("⚠️ DANGER : Êtes-vous sûr de vouloir effacer vos données STAPS ?")) {
        STORE.resetData();
    }
}
...
function skipWarmup() {
    ...
    const parkName = document.getElementById('progress-park-name');
    if (parkName) parkName.innerText = STORE.runtime.session.parcoursObj.nom;
...
// --- 5.3 Instruction Théorique & Progression (Phase C) ---
function loadInstructionPhase() {
    ...
    const progressPercent = Math.round((STORE.runtime.session.stepIndex / totalSteps) * 100);
    
    const activityBar = document.getElementById('activity-progress-bar');
    const activityText = document.getElementById('activity-progress-text');
    
    if (activityBar) activityBar.style.width = progressPercent + "%";
    if (activityText) activityText.innerText = progressPercent;
...
async function startCameraPhase() {
    ...
    try {
        ...
        console.warn("Caméra indisponible ou refusée.", err);
...
window.addEventListener('DOMContentLoaded', () => {
    try {
        // 1. Appliquer le thème (clair/sombre)
        applyTheme();

        // 2. Lier le bouton Mode Nuit sans casser le onclick de la ligne
        const themeToggleDiv = document.querySelector('.setting-item .fa-moon').closest('.setting-item');
        if (themeToggleDiv) {
            themeToggleDiv.onclick = (e) => {
                e.preventDefault();
                toggleTheme();
            };
        }

        // 3. Lier l'événement Entrée pour les champs du profil
        const inputPseudo = document.getElementById('input-edit-pseudo');
        const inputBio = document.getElementById('input-edit-bio');
        if (inputPseudo) inputPseudo.value = STORE.data.profile.pseudo;
        if (inputBio) inputBio.value = STORE.data.profile.bio;

        // 4. Mise à jour de l'UI Globale basée sur le localStorage
        updateAllUI();
        
        // 5. Pré-génération des listes
        renderExplorer(DATABASE.parcours);
        renderSocialFeed();
        
        // 6. Gestion du Splash Screen et Onboarding
        setTimeout(() => {
            ...
            if (!STORE.data.hasCompletedOnboarding) {
                ...
                triggerToast("Connexion Réussie", `Bienvenue sur l'espace STAPS ${STORE.data.profile.pseudo}.`);
            }
        }, 2000); 
